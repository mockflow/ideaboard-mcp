...
var OAUTH_BASE = process.env.MOCKFLOW_OAUTH_URL || 'https://mockflow.com';
var OAUTH_AUTHORIZE = OAUTH_BASE + '/mcp-oauth/';
var OAUTH_TOKEN = OAUTH_BASE + '/ideaboard_oauth_token';
var OAUTH_VALIDATE = process.env.MOCKFLOW_VALIDATE_URL || (OAUTH_BASE + '/ideaboard_oauth_validate');
...
function login() {
	...
	var callbackServer = http.createServer(function(req, res) {
		...
		exchangeCodeForToken(code, function(err, tokenData) {
			...
			validateToken(tokenData.access_token, function(err2, userInfo) {
				...
				console.log('  mockflow-ideaboard-mcp');
				console.log('');
	...
	callbackServer.on('error', function(err) {
		if (err.code === 'EADDRINUSE') {
			console.log('Port ' + CALLBACK_PORT + ' is in use. Close any running mockflow-ideaboard-mcp login and try again.');
		} else {
