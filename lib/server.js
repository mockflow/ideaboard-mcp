	...
	mcpApp.post('/mcp', async function(req, res) {
		try {
			...
			if (method === 'tools/call' && params.name && params.name.startsWith('render_')) {
				try {
					var result = await proxyToBackend(params.name, params.arguments || {}, creds);
 ...
 */
async function proxyToBackend(toolName, args, creds) {
	var actionType = toolName.replace('render_', '');
	var endpoint = BACKEND_URL + '/integrations/gptaction/render_' + actionType;
	...
	// Also include user context in payload for backwards compatibility
	if (creds.userid) {
		payload._oauth = {
			userid: creds.userid,
