	...
	mcpApp.post('/mcp', rateLimiter.middleware, async function(req, res) {
		try {
			...
			if (method === 'tools/call' && params.name && params.name.startsWith('render_')) {
				// Two-step tools answer with reference data when called without their content
				var direct = (typeof REGISTRY.directReply === 'function') ? await REGISTRY.directReply(params.name, params.arguments || {}) : null;
				if (direct) {
					res.json({ jsonrpc: '2.0', id: id, result: { content: [{ type: 'text', text: direct }], isError: false } });
					return;
				}
